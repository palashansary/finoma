...
const EditExpense = () => {
  ...
  // If the user is not logged in or got sign out then page is redirected to the signIn  page
  useEffect(() => {
    if (!user) {
      navigate("/signIn");
    }
  ...
  // Referencing the particular document for the collection using Id
  const expenseCollectionRef = doc(db, "expenses", ExpenseData.id);

  const initialValues = {
    name: ExpenseData.name,
    description: ExpenseData.description,
    category: ExpenseData.category,
    dateOfExpenses: ExpenseData.dateOfExpenses,
      ...
      <div className="h-full flex flex-col items-center justify-center w-[340px] sm:w-[380px] md:w-[420px] lg:w-[440px] px-[10px]  pb-[30px] border rounded-md shadow-md bg-black/[0.01]">
        ...
        >
          ...
          <div className="mt-6 flex justify-between">
            ...
            <button
              type="submit"
              className="px-8 py-1 bg-green-600 text-white rounded-sm flex items-center gap-2 md:gap-4"
            >
              {isLoading && <Spinner />}
              Update Expense
            </button>
